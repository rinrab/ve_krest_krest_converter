...
const рѣдакторъСиПлюсъПлюсъ = CodeJar(странiца.найтиЯщера("#editor-cpp"), (рѣдакторъ) => {
    рѣдакторъ.innerHTML = укротитѣльЯщеровъ.укротить(
...
const рѣдакторъВеКрѣстъКрѣст = CodeJar(странiца.найтиЯщера("#editor-bpp"), (рѣдакторъ) => {
    ...
    рѣдакторъ.innerHTML = translateToV(highlighted);

    // The костыль
    for (elem of рѣдакторъ.querySelectorAll("[династия]")) {
...
var i = 0;
for (const elem of странiца.найтиВсехЯщеров("#examples a")) {
